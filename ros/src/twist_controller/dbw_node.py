...
class DBWNode(object):
    def __init__(self):
        ...
        self.controller = Controller(vehicle_mass, decel_limit, wheel_radius, wheel_base, steer_ratio, min_speed,
        ...
        rospy.Subscriber('/current_velocity', TwistStamped, self.current_veloctiy_cb)
        rospy.get_time()
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            if None not in (self.dbw_enabled, self.proposed_linear_velocity, self.proposed_angular_velocity,
                            self.current_velocity):
                time = rospy.get_time()
                throttle, brake, steering = self.controller.control(self.proposed_linear_velocity,
                                                                    self.proposed_angular_velocity,
                                                                    self.current_velocity, time - prev_time)
                prev_time = time
    ...
    def current_veloctiy_cb(self, twist):
